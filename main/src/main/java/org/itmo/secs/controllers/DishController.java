...
public class DishController {
    ...
    public Mono<ResponseEntity<DishDto>> create(@RequestBody DishCreateDto dishCreateDto) {
        return dishService.save(conversionService.convert(dishCreateDto, Dish.class))
            .map((dish) -> new ResponseEntity<>(
    ...
    public Mono<ResponseEntity<String>> findAll(Integer pageNumber, Integer pageSize) {
        ...
        .map(
            (dishesDto) -> {
                ResponseEntity.ok().header("Content-Type", "application/json").body(jsonConvService.conv(dishesDto))
            }
        );
    }
